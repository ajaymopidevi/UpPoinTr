...
__global__ void chamfer_deviation_kernel(int batch_size,
                                    int n,
                                    const float* xyz1,
                                    int m,
                                    const float* xyz2,
  ...
  for (int i = blockIdx.x; i < batch_size; i += gridDim.x) {
    for (int k2 = 0; k2 < m; k2 += batch) {
      ...
      for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n;
        ...
        if (k2 == 0 || dist[(i * n + j)] > best_dist) {
          dist[(i * n + j)]    = best_dist;
          deviation[(i * n + j) * 3 + 0] = best_distX;
          deviation[(i * n + j) * 3 + 1] = best_distY;
          deviation[(i * n + j) * 3 + 2] = best_distZ;
          indexes[(i * n + j)] = best_dist_index;
        }
      }
      __syncthreads();
...
std::vector<torch::Tensor> chamfer_deviation_cuda(torch::Tensor xyz1,
  ...
  torch::Tensor deviation1 =
    torch::zeros({batch_size, n, 3}, torch::CUDA(torch::kFloat));
  torch::Tensor dist2 =
    torch::zeros({batch_size, m}, torch::CUDA(torch::kFloat));
  torch::Tensor deviation2 =
    torch::zeros({batch_size, m, 3}, torch::CUDA(torch::kFloat));
